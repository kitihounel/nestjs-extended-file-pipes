...
import { ParseFileFieldsPipe } from './parse-file-fields.pipe';
...
describe('ParseFileFieldsPipe', () => {
  ...
  describe('transform', () => {
    it('only ouputs files specified in the options', async () => {
      ...
      expect(Object.keys(result)).toEqual(['documents', 'images']);
    });
    ...
    it('should throw when required file is not present', () => {
      ...
      return expect(
        pipe.transform({ images, documents })
    ...
    it('should throw when a file does not pass validation', () => {
      ...
      expect(pipe.transform({ documents, extras })).rejects.toThrow(
        customErrorMessage
      );
    });
  });
  ...
  describe('mergeWithCommonOptions', () => {
    describe('commonOptions', () => {
      it('returns common options when there no field specific options', () => {
        ...
        expect(pipe.mergeWithCommonOptions({})).toEqual({
          ...commonOptions,
    ...
    describe('fileIsRequired', () => {
      it('is by default true', () => {
        ...
        expect(mergedOptions.fileIsRequired).toBe(true);
      });
      ...
      it('gives priority to field specific value', () => {
        ...
        expect(mergedOptions.fileIsRequired).toBe(false);
      });
    });
    ...
    describe('validators', () => {
      it('commonValidators are listed first', () => {
        ...
        expect(validators?.[0]).toBe(alwaysValid);
      });
      ...
      it('validators are correctly merged', () => {
        ...
        expect(validators).toEqual([alwaysInvalid, alwaysValid]);
      });
    });
    ...
    describe('errorHttpStatusCode and exceptionFactory', () => {
      it('defaults to "undefined"', () => {
        ...
        expect(mergedOptions.errorHttpStatusCode).toEqual(undefined);
        expect(mergedOptions.exceptionFactory).toEqual(undefined);
      });

      it('field specific options have priority', () => {
        const customExceptionFactory = (message: string) => new Error(message);
        const pipe = new ParseFileFieldsPipe({
          commonOptions: {
            errorHttpStatusCode: 400,
          },
        ...
        expect(mergedOptions.errorHttpStatusCode).toBe(422);
        expect(mergedOptions.exceptionFactory).toBe(customExceptionFactory);
      });
    });
  });
});
